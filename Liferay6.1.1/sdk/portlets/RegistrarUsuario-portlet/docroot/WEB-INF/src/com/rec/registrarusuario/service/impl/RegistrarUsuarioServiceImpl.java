...
public class RegistrarUsuarioServiceImpl implements RegistrarUsuarioService {
	...
	private static Log LOG = LogFactoryUtil
			.getLog(RegistrarUsuarioServiceImpl.class);

	@SuppressWarnings("deprecation")
	@Override
	public User registrarUsuarioPostulante(long creatorUserId, long companyId,
			...
			Map<String, String> camposExtra, ServiceContext serviceContext)
			throws DuplicateUserEmailAddressException, SystemException,
			PortalException, DuplicateUserDNIException {
		...
		dynamicQuery.add(PropertyFactoryUtil.forName(
				ConstantesUtil.EMAILADDRESS).eq(emailAddress));
		...
		dynamicQuery = DynamicQueryFactoryUtil
				.forClass(ExpandoValue.class,
						PortalClassLoaderUtil.getClassLoader())
				.add(PropertyFactoryUtil.forName(ConstantesUtil.COLUMNID).eq(
						GetterUtil.getLong(column.getColumnId())))
				.add(PropertyFactoryUtil.forName(ConstantesPortalUtil.DATA).eq(
						camposExtra.get(ConstantesUtil.DNI)))
				.add(PropertyFactoryUtil.forName(ConstantesUtil.CLASSNAMEID)
						.eq(GetterUtil.getLong(ClassNameLocalServiceUtil
								.getClassNameId(User.class.getName()))));
		long dniExists = ExpandoValueLocalServiceUtil
		...
		dynamicQuery = DynamicQueryFactoryUtil.forClass(User.class,
				PortalClassLoaderUtil.getClassLoader());
		dynamicQuery.add(PropertyFactoryUtil.forName(ConstantesUtil.SCREENNAME)
				.eq(screenName));
...
//				passwordPolicy);
		...
		User nuevoPostulante = UserLocalServiceUtil.addUser(creatorUserId,
	...
	@Override
	public int validarUsuarioNuevoPostulante(String usuario_postulante)
			throws SystemException {
		DynamicQuery dynamicQuery = DynamicQueryFactoryUtil.forClass(
				User.class, PortalClassLoaderUtil.getClassLoader());
		dynamicQuery.add(PropertyFactoryUtil.forName(ConstantesUtil.SCREENNAME)
				.like(ConstantesPortalUtil.PERCENT + usuario_postulante
						+ ConstantesPortalUtil.PERCENT));
		long userexists = UserLocalServiceUtil.dynamicQueryCount(dynamicQuery);
		LOG.debug("Existe Screenname : " + userexists);
